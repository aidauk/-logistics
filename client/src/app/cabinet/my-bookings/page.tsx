...
import { selectOrders } from "@/redux/features/order/slices/orders.slice";
import { statusTypes } from "@/redux/status-types";
import { fetchMyCreatedOrder, fetchMyOrders } from "@/api/orders";
import Link from "next/link";

const page = () => {
  const accessToken = useLocalStorage("", "accessToken");
  const dispatch: AppDispatch = useDispatch();
  const { data, status, error } = useSelector(selectOrders);

  useEffect(() => {
    if (status === statusTypes.INIT) {
      dispatch(fetchMyOrders(accessToken));
    }
  }, [status, dispatch]);
  ...
  const ordersInfo = data.map((order) => {
    const { booking_info, product } = order || {};
    return [
      { label: "First Name", value: booking_info?.name },
      { label: "Last Name", value: booking_info?.lastName },
      { label: "Phone Number", value: booking_info?.phone },
      { label: "Email", value: booking_info?.email },
      { label: "Entry Date", value: booking_info?.entry_date },
      { label: "Leaving Date", value: booking_info?.leaving_date },
      { label: "Guests", value: booking_info?.guests },
      { label: "Status", value: order?.state },
      { label: "Product", link: product ? `/products/${product._id}` : null },
    ];
  });
                ...
                {order.map((info, i) => (
                  <div
                    ...
                    className={`flex w-full items-center justify-between ${
                      i !== order.length - 1 && "border-b border-b-gray-7"
                    } py-[6px]`}
                  >
                    <p className="w-1/2">{info.label}:</p>
                    {info.label === "Product" && info.link ? (
                      <Link href={info.link} className="hover:text-primary">
                        <svg
