...
export default function SigninWithPassword() {
  ...
  const handleLoginSubmit = async (formData: FormData) => {
    try {
      const response = await Adminlogin(formData); // Call your login function

      if (!response?.accessToken) {
        <AlertError title="Error" message="You are not an admin"/>
      } else {
  ...
  return (
    <form
      onSubmit={async (event) => {
        ...
        const formData = new FormData(event.target as HTMLFormElement); // Create FormData from form
        await handleLoginSubmit(formData); // Call the separate function
      }}
    >
      <div className="mb-4">
        <label
