...
const TableTwo = ({ products }: { products: ProductInterface[] | null }) => {
  ...
  const handleVerificatingProduct = async (productId: string) => {
    ...
    if (confirmed) {
      try {
        const updatedProduct = await verifyProduct(productId, token);

        if (updatedProduct) {
          alert("Product verified successfully!");
          console.log("Verified product data:", updatedProduct);
          window.location.reload();
        } else {
          alert("Failed to verify the product.");
        }
      } catch (error) {
      ...
      alert("Product verification canceled.");
    }
  };
  const handleDisactivatingProduct = async (productId: string) => {
    const confirmed = window.confirm(
      "Are you sure you want to disactivate this product?",
    );

    if (confirmed) {
      setLoading(true);
      try {
        const disactivatedProduct = await disactiveProduct(productId, token);

        if (disactivatedProduct) {
          alert("Product disactivated successfully!");
          console.log("Disactivated product data:", disactivatedProduct);
          window.location.reload();
        } else {
          alert("Failed to disactivate the product.");
        }
      } catch (error) {
        alert("An error occurred during the disactivation.");
      } finally {
        setLoading(false);
      }
    } else {
      alert("Product disactivation canceled.");
    }
  };
    ...
    if (confirmed) {
      ...
      try {
        ...
        if (response?.status == 200) {
          ...
          window.location.reload();
          console.log("Deleted product data:", response);
        } else {
      ...
      alert("Product deleting canceled.");
    }
  };
        ...
        >
          ...
          <div className="col-span-2 flex items-center  justify-end">
            <div className="flex items-center justify-end space-x-3.5">
              <button
                onClick={() => handleVerificatingProduct(product._id)}
              ...
              <Link
                href={`/products/${product._id}/preview`}
                className="hover:text-primary"
              >
                <svg
              ...
              <button
                onClick={() => {
                  handleDeletingProduct(product._id);
                }}
                className="hover:text-primary"
              >
                <svg
              ...
              <button
                onClick={() => handleDisactivatingProduct(product._id)}
                className="hover:text-primary"
              >
                <svg
                  aria-hidden="true"
