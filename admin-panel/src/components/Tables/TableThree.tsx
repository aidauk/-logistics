...
import { deleteApplication, updateAppToAnswered } from "@/api/applications";
...
const TableThree = ({
  ...
  const handleUpdateToAnswered = async (appId: string) => {
    const token = localStorage.getItem("accessToken");
    try {
      const response = await updateAppToAnswered(token, appId);

      if (response.status == 200) {
        alert("Application verified successfully!");
        window.location.reload();
      } else {
        alert("Failed to verify the Application.");
      }
    } catch (error) {
  ...
  const handleDeletingApp = async (appId: string) => {
    const token = localStorage.getItem("accessToken");
    ...
    if (confirmed) {
      try {
        const response = await deleteApplication(token, appId);
        console.log("🚀 ~ handleDeletingProduct ~ response:", response);

        if (response?.status == 200) {
          alert("Application deleted successfully!");
          window.location.reload();
          console.log("Deleted application data:", response);
        } else {
              ...
              <tr key={index}>
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <Link
                      href={`/applications/${application._id}/preview`}
                    ...
                    <button
                      className="hover:text-primary"
                      onClick={() => handleDeletingApp(application._id)}
                    >
                      <svg
                        className="fill-current"
                        width="20"
                    ...
                    <button
                      onClick={() => handleUpdateToAnswered(application._id)}
                      className="hover:text-primary"
                    >
                      <svg
                        aria-hidden="true"
