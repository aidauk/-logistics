"use client";
import Image from "next/image";
import CustomSelect from "../CustomSelect";
import { useEffect, useState } from "react";
import { OrderInterface, UserInterface } from "@/interfaces";
import { deleteUser, fetchUsers } from "@/api/users";
import Link from "next/link";
import { archiveOrder, cancelOrder, fetchOrders, payOrder } from "@/api/orders";

const TableFour = () => {
  const [selectedValue, setSelectedValue] = useState<string>("");
  const [orders, setOrders] = useState<OrderInterface[] | null>();
  const [token, setToken] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    const token = localStorage.getItem("accessToken");
    setToken(token);
  }, []);

  const handleSelectChange = (value: string) => {
    setSelectedValue(value);
  };

  const handleVerificatingOrder = async (orderId: string) => {
    const confirmed = window.confirm(
      "Are you sure you want to verify this order?",
    );

    if (confirmed) {
      try {
        const updatedOrder = await payOrder(token, orderId);

        if (updatedOrder) {
          alert("Order verified successfully!");
          console.log("Verified order data:", updatedOrder);
          window.location.reload();
        } else {
          alert("Failed to verify the order.");
        }
      } catch (error) {
        alert("An error occurred during the verification.");
      } finally {
        setLoading(false);
      }
    } else {
      alert("Order verification canceled.");
    }
  };

  const handleCancelingOrder = async (id: string) => {
    const confirmed = window.confirm(
      "Are you sure you want to cancel this order?",
    );

    if (confirmed) {
      setLoading(true);
      try {
        const canceledOrder = await cancelOrder(token, id);

        if (canceledOrder) {
          alert("Order canceled successfully!");
          window.location.reload();
          console.log("Canceled order data:", canceledOrder);
        } else {
          alert("Failed to cancel the order.");
        }
      } catch (error) {
        alert("An error occurred during canceling the order.");
      } finally {
        setLoading(false);
      }
    } else {
      alert("Order could not be canceled.");
    }
  };

  const handleArchivingOrder = async (id: string) => {
    const confirmed = window.confirm(
      "Are you sure you want to archive this order?",
    );

    if (confirmed) {
      setLoading(true);
      try {
        const canceledOrder = await archiveOrder(token, id);

        if (canceledOrder) {
          alert("Order archived successfully!");
          window.location.reload();
          console.log("Archived order data:", canceledOrder);
        } else {
          alert("Failed to arvchive the order.");
        }
      } catch (error) {
        alert("An error occurred during archiving the order.");
      } finally {
        setLoading(false);
      }
    } else {
      alert("Order could not be archived.");
    }
  };

  const options = [
    { value: "6602be9524279ec4e26a952d", label: "Квартира" },
    { value: "668bdf92ed93cc7f62552116", label: "Котедж" },
    { value: "668bdf9ced93cc7f62552117", label: "Дача" },
  ];

  useEffect(() => {
    const getOrdersAndItsUser = async () => {
      try {
        const data = await fetchOrders(token);
        setOrders(data ?? null); // Assuming response.data holds the Orders data
      } catch (err) {
        console.error(err);
      } finally {
        return <div>Loading...</div>; // Stop loading after the fetch completes (success or error)
      }
    };

    getOrdersAndItsUser();
  }, []);

  if (orders?.length == 0) {
    return <h1>No orders found</h1>;
  }

  return (
    <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
      <div className="mb-[30px]">
        <CustomSelect
          type={3}
          options={options}
          onChange={handleSelectChange}
          placeholder="Filter Users"
        />
      </div>

      <div className="flex flex-col">
        <div className="grid grid-cols-5 sm:grid-cols-5">
          <div className="col-span-1 px-2 pb-3.5">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              username
            </h5>
          </div>
          <div className="col-span-1 px-2 pb-3.5 text-start">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              created
            </h5>
          </div>
          <div className="col-span-1 hidden px-2 pb-3.5 text-start sm:block">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              status
            </h5>
          </div>
          <div className="col-span-1 hidden px-2 pb-3.5 text-start sm:block">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              booking-date
            </h5>
          </div>
          <div className="col-span-1 hidden px-2 pb-3.5 text-end sm:block">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              actions
            </h5>
          </div>
        </div>

        {orders?.map((order, key) => (
          <div
            className={`grid grid-cols-5 sm:grid-cols-5 ${
              key === orders.length - 1
                ? ""
                : "border-b border-stroke dark:border-dark-3"
            }`}
            key={key}
          >
            <div className="col-span-1 flex items-center px-2 py-4">
              <p className="hidden font-medium text-dark dark:text-white sm:block">
                {order.booking_info?.name}
              </p>
            </div>

            <div className="col-span-1 flex items-center justify-start px-2 py-4">
              <p className="font-medium text-dark dark:text-white">
                {new Date(order.createdAt).toDateString()}
              </p>
            </div>

            <div className="col-span-1 flex items-center justify-start px-2 py-4">
              <p
                className={`inline-flex rounded-full px-3.5 py-[4px] text-body-sm font-medium ${
                  order.state == "active"
                    ? "bg-[#219653]/[0.08] text-[#219653]"
                    : order.state == "canceled"
                      ? "bg-[#D34053]/[0.08] text-[#D34053]"
                      : order.state == "pending"
                        ? "bg-[#FFA70B]/[0.08] text-[#FFA70B]"
                        : "bg-[#FFA70B]/[0.08] text-primary"
                }`}
              >
                {order.state}
              </p>
            </div>

            <div className="col-span-1 flex items-center justify-start px-2 py-4">
              <p className="font-medium text-dark dark:text-white">
                {new Date(order.booking_info.entry_date).toLocaleDateString()}
                {" - "}
                {new Date(order.booking_info.leaving_date).toLocaleDateString()}
              </p>
            </div>

            <div className="col-span-1 hidden items-center justify-end gap-[10px] px-2 py-4 sm:flex">
              <button
                onClick={() => handleVerificatingOrder(order._id)}
                className="hover:text-primary"
              >
                <svg
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  fill="none"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke="currentColor"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M15 4h3a1 1 0 0 1 1 1v15a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h3m0 3h6m-6 7 2 2 4-4m-5-9v4h4V3h-4Z"
                  />
                </svg>
              </button>
              <Link
                href={`/orders/${order._id}/preview`}
                className="hover:text-primary"
              >
                <svg
                  className="fill-current"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.99935 6.87492C8.27346 6.87492 6.87435 8.27403 6.87435 9.99992C6.87435 11.7258 8.27346 13.1249 9.99935 13.1249C11.7252 13.1249 13.1243 11.7258 13.1243 9.99992C13.1243 8.27403 11.7252 6.87492 9.99935 6.87492ZM8.12435 9.99992C8.12435 8.96438 8.96382 8.12492 9.99935 8.12492C11.0349 8.12492 11.8743 8.96438 11.8743 9.99992C11.8743 11.0355 11.0349 11.8749 9.99935 11.8749C8.96382 11.8749 8.12435 11.0355 8.12435 9.99992Z"
                    fill=""
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.99935 2.70825C6.23757 2.70825 3.70376 4.96175 2.23315 6.8723L2.20663 6.90675C1.87405 7.3387 1.56773 7.73652 1.35992 8.20692C1.13739 8.71064 1.04102 9.25966 1.04102 9.99992C1.04102 10.7402 1.13739 11.2892 1.35992 11.7929C1.56773 12.2633 1.87405 12.6611 2.20664 13.0931L2.23316 13.1275C3.70376 15.0381 6.23757 17.2916 9.99935 17.2916C13.7611 17.2916 16.2949 15.0381 17.7655 13.1275L17.792 13.0931C18.1246 12.6612 18.431 12.2633 18.6388 11.7929C18.8613 11.2892 18.9577 10.7402 18.9577 9.99992C18.9577 9.25966 18.8613 8.71064 18.6388 8.20692C18.431 7.73651 18.1246 7.33868 17.792 6.90673L17.7655 6.8723C16.2949 4.96175 13.7611 2.70825 9.99935 2.70825ZM3.2237 7.63475C4.58155 5.87068 6.79132 3.95825 9.99935 3.95825C13.2074 3.95825 15.4172 5.87068 16.775 7.63475C17.1405 8.10958 17.3546 8.3933 17.4954 8.71204C17.627 9.00993 17.7077 9.37403 17.7077 9.99992C17.7077 10.6258 17.627 10.9899 17.4954 11.2878C17.3546 11.6065 17.1405 11.8903 16.775 12.3651C15.4172 14.1292 13.2074 16.0416 9.99935 16.0416C6.79132 16.0416 4.58155 14.1292 3.2237 12.3651C2.85821 11.8903 2.64413 11.6065 2.50332 11.2878C2.37171 10.9899 2.29102 10.6258 2.29102 9.99992C2.29102 9.37403 2.37171 9.00993 2.50332 8.71204C2.64413 8.3933 2.85821 8.10958 3.2237 7.63475Z"
                    fill=""
                  />
                </svg>
              </Link>
              <button
                onClick={() => handleCancelingOrder(order._id)}
                className="hover:text-primary"
              >
                <svg
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
                  height="18"
                  fill="none"
                  viewBox="0 1 23 23"
                >
                  <path
                    stroke="currentColor"
                    stroke-linecap="round"
                    stroke-width="1.5"
                    d="m6 6 12 12m3-6a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
                  />
                </svg>
              </button>
              <button
                onClick={() => handleArchivingOrder(order._id)}
                className="hover:text-primary"
              >
                <svg
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 1 21 21"
                  fill="none"
                >
                  <path
                    stroke="currentColor"
                    stroke-linejoin="round"
                    stroke-width="1.5"
                    d="M10 12v1h4v-1m4 7H6a1 1 0 0 1-1-1V9h14v9a1 1 0 0 1-1 1ZM4 5h16a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z"
                  />
                </svg>
              </button>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default TableFour;
