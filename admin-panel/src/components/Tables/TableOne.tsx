...
const TableOne = () => {
  ...
  const handleDeletingUser = async (userId: string) => {
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        ...
        setUsers(data ?? null); // Assuming response.data holds the product data
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-3 sm:grid-cols-5">
          ...
          <div className="col-span-2 hidden px-2 pb-3.5 text-end sm:block">
            <h5 className="text-sm font-medium uppercase xsm:text-base">
              Actions
            </h5>
          </div>
        ...
        {users?.map((user, key) => (
          ...
          >
            ...
            <div className="col-span-2 hidden items-center justify-end gap-[10px] px-2 py-4 sm:flex">
              <Link
                href={`/users/${user._id}/preview`}
