    ...
    <>
      <li>
        <Link
          href={item.route}
          onClick={handleClick}
          className={`${pageName === item.label.toLowerCase() ? "bg-primary/[.07] text-primary dark:bg-white/10 dark:text-white" : "text-white hover:bg-gray-2 hover:text-dark dark:text-gray-5 dark:hover:bg-white/10 dark:hover:text-white"} group relative flex items-center gap-3 rounded-[7px] font-medium`}
        >
          <div className="flex items-center h-full w-full px-3.5 py-3 opacity-[0.8] hover:opacity-[1] gap-[10px]">
            {item.icon}
            {item.label}
          </div>
          {item.message && (
          ...
          {item.children && (
            <svg
              className={`absolute right-3.5 top-1/2 z-[-1] -translate-y-1/2 fill-current ${
