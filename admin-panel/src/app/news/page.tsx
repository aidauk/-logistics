...
import { deleteSigleNews, fetchNews } from "@/api/news";
...
const News = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        setNews(data ?? null); // Assuming response.data holds the product data
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const handleDeletingProduct = async (newsId: string) => {
    ...
    if (confirmed) {
      try {
        const response = await deleteSigleNews(token, newsId);
        console.log("🚀 ~ handleDeletingProduct ~ response:", response);

        if (response?.status == 200) {
          alert("News deleted successfully!");
          window.location.reload();
          console.log("Deleted News data:", response);
        } else {
          alert("Failed to delete the News.");
        }
      } catch (error) {
        alert("An error occurred during deleting.");
      }
    } else {
      alert("News deleting canceled.");
    }
  };
  ...
  return (
    <DefaultLayout>
      ...
      <div className="grid grid-cols-1 gap-[20px] xs:grid-cols-2 sm:grid-cols-3 xl:grid-cols-5">
        <div className="col-span-1 mt-[20px] flex items-center justify-center rounded-[10px] border border-stroke p-8 dark:border-dark-3 dark:shadow-card">
          <Link href={`/news/add`} className="rounded-[10px] border border-stroke bg-white p-4 text-center shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
            <svg
        ...
        {news?.map((item, key) => {
          return (
            <div
              key={key}
              className="relative col-span-1 mt-[20px] rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card"
            >
              <span className="absolute z-9 flex h-full w-full items-center justify-center gap-[20px] rounded-[10px] bg-gray-dark opacity-0 hover:opacity-80">
                <Link
                  href={`/news/${item._id}/preview`}
                  className="hover:text-primary"
                >
                  <svg
                    className="h-[22px] w-[22px] fill-current sm:h-[26px] sm:w-[26px]"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                ...
                <button onClick={() => handleDeletingProduct(item._id)} className="hover:text-primary">
                  <svg
                    className="h-[22px] w-[22px] fill-current sm:h-[26px] sm:w-[26px]"
                    viewBox="0 0 20 20"
                    fill="none"
                ...
                <Link
                  href={`/news/${item._id}/edit`}
                  className="hover:text-primary"
                >
                  <svg
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
                    className="sm:h-[26 px] h-[22px] w-[22px] sm:w-[26px]"
                    fill="none"
                    viewBox="0 0 22 22"
                  >
                    <path
              ...
              <div className="relative z-0 w-full h-[200px] overflow-hidden rounded-t-[10px]">
                <Image
                  className="w-full he-full object-cover rounded-t-[10px]"
                  src={item.images[0].uri ?? defaultImage}
                  alt="default-image"
                  fill
                />
              </div>
              <div className="px-4 py-2 sm:px-7.5 sm:py-4">
